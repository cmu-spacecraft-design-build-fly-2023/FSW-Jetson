...
def print_detected_landmarks(detection_results):
    ...
    # Header
    print(f"{'Class':>10} {'Centroid X':>10} {'Centroid Y':>10} {'Centroid Lat':>15} {'Centroid Lon':>15}")
    ...
    for landmark_cls, cent_xy, cent_latlon in zip(landmark_classes, centroid_xy, centroid_latlons):
        print(f"{landmark_cls:>10.2f} {cent_xy[0]:>10.2f} {cent_xy[1]:>10.2f} {cent_latlon[0]:>15.6f} {cent_latlon[1]:>15.6f}")
...
def evaluate_inference(image_file, detection_results, image_path, label_path, csv_writer, output_dir, draw_boxes_flag):
    """
    ...
    ground_truth_boxes, ground_truth_centroids = read_ground_truth(label_path, detected_classes, image_width, image_height)

    # Calculate errors
    detected_class_centroids = [(cls, centroid) for cls, centroid in zip(detected_classes, detected_centroids)]
    ...
    for (detected_cls, cent_xy), cent_latlon in zip(detected_class_centroids, centroid_latlons):
        class_error = class_errors.get(detected_cls, float('nan'))  # Fetch the error for this specific class
        gt_centroid = next((gt_cent for gt_cls, gt_cent in ground_truth_centroids if gt_cls == detected_cls), [float('nan'), float('nan')])
        csv_writer.writerow([
...
def run_ld_inference_test(region_id, draw_boxes_flag=False, data_path=None):
   ...
   # Directory setup
    ...
    image_files = [f for f in os.listdir(images_dir) if isfile(join(images_dir, f)) and f.endswith(('.png', '.jpg', '.jpeg'))]
    ...
    with open(csv_file_path, 'w', newline='') as csv_file:
        ...
        headers = ['Image', 'Groundtruth X', 'Groundtruth Y', 'Centroid X', 'Centroid Y', 'Centroid Lat', 'Centroid Lon', 'Class Error', 'Mean Error']
        ...
        for image_file in image_files:
            ...
            evaluate_inference(image_file, detection_results, image_path, label_path, csv_writer, output_dir, draw_boxes_flag)
            
            print(f"Inference results saved to {csv_file_path}")
            print("----------")

def run_ld_inference(image_path, region_id, label_path, draw_boxes_flag=False, output_dir='inference_output/'):
    """
    ...
    # CSV file setup
    csv_file_path = os.path.join(output_dir, f'{os.path.basename(image_path).split(".")[0]}_inference_result.csv')
    with open(csv_file_path, 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        # Write the header row
        headers = ['Image', 'Groundtruth X', 'Groundtruth Y', 'Centroid X', 'Centroid Y', 'Centroid Lat', 'Centroid Lon', 'Class Error', 'Mean Error']
        ...
        print_detected_landmarks(detection_results)
        evaluate_inference(os.path.basename(image_path), detection_results, image_path, label_path, csv_writer, output_dir, draw_boxes_flag)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--region', required=True, help='Region ID to test specific LD inference')
